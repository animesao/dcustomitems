...
## 1. Сообщения в версии 1.320.278
...
Этот файл является редактируемым шаблоном и шпаргалкой с названиями полей. В версии `1.320.278` runtime-компилятор не вызывает метод `load()` автоматически, поэтому простое изменение `items/messages.java` не изменит сообщения после `/ci reload`.
