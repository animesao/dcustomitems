...
## 1. Messages in version 1.320.280
...
That file is an editable reference template containing message field names. In version `1.320.280`, the runtime compiler does not automatically call a `load()` method, so editing `items/messages.java` alone does not change messages after `/ci reload`.
